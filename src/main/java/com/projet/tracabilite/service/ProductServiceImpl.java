...
public class ProductServiceImpl implements ProductService {
        ...
        public Product createProduct(Long typeId, Long materialId, Double quantity, Long supplierId,
                ...
                StepType initialStepType = stepTypeRepository.findById(initialStepId)
                                .orElseThrow(() -> new RuntimeException(
                                                "Type d'étape introuvable avec l'ID : " + initialStepId));

                Status initialStatus = statusRepository.findByName(initialStepType.getName())
                                .orElseThrow(
                                                () -> new RuntimeException("Statut introuvable en base pour : "
                                                                + initialStepType.getName()));
                ...
                // ==========
                java.time.LocalDateTime baseDate = collectionDate != null ? collectionDate
                ...
                for (int i = 0; i < mandatoryStepNames.length; i++) {
                        ...
                        // Chaque étape reçoit une date décalée de +i minutes pour garantir l'ordre
                        // chrono
        ...
        public ProductDTO updateProduct(Long id, UpdateProductRequest request) {
                ...
                if (request.getTypeId() != null) {
                        ProductType type = productTypeRepository.findById(request.getTypeId())
                                        .orElseThrow(() -> new RuntimeException(
                                                        "Type de produit introuvable avec l'ID : "
                                                                        + request.getTypeId()));
        ...
        public void deleteProduct(Long id) {
                ...
                // 1. Créer ou récupérer le statut "ANNULÉ"
                Status cancelStatus = statusRepository.findByName("ANNULÉ")
                                .orElseGet(() -> {
                                        Status s = Status.builder().name("ANNULÉ").build();
                                        return statusRepository.save(s);
                                });

                // 2. Changer le statut du produit
                product.setCurrentStatus(cancelStatus);

                // 3. Ajouter une étape de traçabilité pour marquer l'annulation
                StepType cancelStepType = stepTypeRepository.findByName("ANNULATION")
                                .orElseGet(() -> {
                                        StepType type = StepType.builder().name("ANNULATION").build();
                                        return stepTypeRepository.save(type);
                                });

                TraceabilityStep cancelStep = new TraceabilityStep();
                cancelStep.setProduct(product);
                cancelStep.setType(cancelStepType);
                cancelStep.setDate(java.time.LocalDateTime.now());
                cancelStep.setLocation("Système");
                cancelStep.setDescription("Lot annulé et retiré du stock actif par l'opérateur.");
                stepRepository.save(cancelStep);

                // 4. Sauvegarder les modifications du produit
                productRepository.save(product);
        }
        ...
        public ProductDTO addStepToProduct(Long productId, AddStepRequest request) {
                ...
                // Mettre à jour le statut du produit pour TOUTES les étapes (obligatoires ou
                // sur-mesure)
        ...
        private ProductDTO mapToDTO(Product product) {
                ...
                return ProductDTO.builder()
                                ...
                                .currentStatus(product.getCurrentStatus() != null ? product.getCurrentStatus().getName()
                                                : null)
